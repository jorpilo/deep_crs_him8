...
from denoise_and_cluster.py import sharpen
...
def train(data_filename1, cluster_filename, prediction):
    ds = xr.open_dataset(data_filename1)
    print(ds['B7'].shape)
    data = ds['B7'].shape[0]
    print(ds)
    ...
    ds = xr.open_dataset(cluster_filename)
    print(ds['K7'].shape)
    ...
    ds = xr.open_dataset(prediction)
    print(ds)
    y2 = ds['NP'].values[1:, 0:200, 0:200, np.newaxis]
    print('loaded Numerical Predictions')
    y = ds['TP'].values[1:, 0:200, 0:200, np.newaxis]
