...
def GetModel(in_frames):
    ...
    ofconv = BatchNormalization(axis=-1)(ofconv)
    ofconv = Conv3D(32, (in_frames, 3, 3), strides=(1, 1, 1), activation='relu', padding='same',data_format="channels_last")(ofconv)
    ofconv = BatchNormalization(axis=-1)(ofconv)
    ofconv = Conv3D(16, (in_frames, 3, 3), strides=(1, 1, 1), activation='relu', padding='same',data_format="channels_last")(ofconv)
    ...
    imconv = BatchNormalization(axis=-1)(imconv)
    imconv = Conv3D(32, (in_frames, 3, 3), strides=(1, 1, 1), activation='relu', padding='same',data_format="channels_last")(imconv)
    ...
    stpnconv = Conv3D(64, (in_frames, 3, 3), strides=(1, 1, 1), activation='relu', padding='same', data_format="channels_last")(imconv)
    stpnconv = BatchNormalization(axis=-1)(stpnconv)
    stpnconv = Conv3D(128, (in_frames, 3, 3), strides=(1, 1, 1), activation='relu', padding='same', data_format="channels_last")(stpnconv)
    stpnconv = BatchNormalization(axis=-1)(stpnconv)
    stpnconv = Conv3D(64, (in_frames, 3, 3), strides=(1, 1, 1), activation='relu', padding='same', data_format="channels_last")(stpnconv)
    stpnconv = BatchNormalization(axis=-1)(stpnconv)
    stpnconv = Conv3D(32, (in_frames, 3, 3), strides=(1, 1, 1), activation='relu', padding='same', data_format="channels_last")(stpnconv)
    stpnconv = BatchNormalization(axis=-1)(stpnconv)
    stpnconv = Conv3D(4, (in_frames, 3, 3), strides=(1, 1, 1), activation='relu', padding='same', data_format="channels_last")(stpnconv)
    stpnconv = MaxPooling3D(pool_size=(in_frames, 1, 1), strides=None, padding='same', data_format='channels_last')(stpnconv)
    stpnconv = Lambda(lambda x: x[:, 0, :, :, :])(stpnconv)
    ...
    # stpnconv = BatchNormalization(axis=-1)(stpnconv)
    # stpnconv = ConvLSTM2D(64, (3, 3), strides=(1, 1), activation='relu', padding='same', data_format="channels_last", return_sequences=True)(stpnconv)
    # stpnconv = BatchNormalization(axis=-1)(stpnconv)
...
def main():
    ...
    mc = ModelCheckpoint('bestSTPN2_weights.h5',monitor='val_loss', mode='min', verbose=1, save_best_only=True, save_weights_only=True)
    model.fit_generator(MyGenerator_Train, epochs=5, verbose=1, shuffle=False,
                                  validation_data=MyGenerator_Test,
                                  use_multiprocessing=True, workers=4, callbacks=[es])
    # print('saving model')
    # model_json = model.to_json()
    # with open('STPN2_model.json', "w") as json_file:
    #     json_file.write(model_json)
    # json_file.close()
    # model.save_weights('STPN2_weights.h5')

    X,Y = MyGenerator_Train.__getitem__(0)
    #model = load_model_json('STPN2_model.json', 'bestSTPN2_weights.h5')

    Y_pred = model.predict(X)
    print(Y_pred.shape)
    print(Y_pred[7, :, :, 2])
    print(Y[7, :, :,2])
    plt.imsave("pred_STPN2.png", np.exp(Y_pred[7, :, :, 2]) - 1)
    plt.imsave("real_STPN2.png", np.exp(Y[7, :, :,2]) - 1)
...
def predict_frame():
    ...
    in_frames = 3
    xtrainID, ytrainID = dataGenerator(time, in_frames, 1)
    MyGenerator_Train = Generator2D(xtrainID, ytrainID, images, opticalflow, batch_size=8)
